...
public class _TimerEJBHome_Stub extends Stub implements TimerEJBHome {
    
    private static final String[] _type_ids = {
        "RMI:com.timer.ejb.TimerEJBHome:0000000000000000", 
    ...
    public TimerEJB create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (TimerEJB) in.read_Object(TimerEJB.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",com.timer.ejb.TimerEJBHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    TimerEJB result = ((com.timer.ejb.TimerEJBHome)so.servant).create();
                    return (TimerEJB)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
