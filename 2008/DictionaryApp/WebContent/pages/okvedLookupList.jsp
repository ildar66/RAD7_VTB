...
<%
 	...
 	OkvedLookupListForm listForm = (OkvedLookupListForm)session.getAttribute("OkvedLookupListForm");
 	String sortBy = listForm.getOrderBy();
 	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.OKVED_LOOKUP_LIST);
...
<html:form action="/okvedLookupList.do" styleId="listForm">
<html:errors/>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
    ...
    <!-- Table body -->
    <logic:iterate id="item" name="<%=BeanKeys.OKVED_LOOKUP_LIST%>" type="com.vtb.custom.OkvedTO">
        <logic:present name="item">
            ...
            <logic:notPresent name="item" property="spoID">
            	<tr id='crm<bean:write name="item" property="crmID" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
	                ...
	                <td class="text"><bean:write name="item" property="crmID" filter="true"/></td>
	                <td class="text">
						<A href="javascript:Go('<bean:write name="item" property="spoID" filter="true"/>|<bean:write name="item" property="name" filter="true"/>|<bean:write name="item" property="crmID" filter="true"/>')" title="Выберите запись CRM">	                
		 					<bean:write name="item" property="name" filter="true"/></A>
	                </td>
	                <td class="text">
