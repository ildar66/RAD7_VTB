...
<HEAD>
	...
	<TITLE>Справочник Штрафных санкций</TITLE>
</HEAD>
<BODY class="content">
<%
	java.lang.String gColSpan = "4";
	PunitiveMeasureListForm listForm = (PunitiveMeasureListForm)session.getAttribute("PunitiveMeasureListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.PUNITIVE_MEASURE_LIST);
...
<html:form action="/punitiveMeasureList.do" styleId="listForm" focus="searchStr">
	<html:errors />
	...
	<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1">
		...
		<tr CLASS="layout-manager">
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
				<TR>
					<TD class="find-filter">
						<% if(canEdit) { %>
							<html:link page="/punitiveMeasureDialog.do?action=Add">
		...
		<tr class="title">
			...
			<td class="column-head-name" align="center" valign="middle"><A
				...
				href="javascript:SortBy('name')">Наименование санкции</A>
			<%=isUp(sortBy, "name")%></td>
			<td class="column-head-name" align="center" valign="middle"><img
				src="<%=request.getContextPath()%>/images/empty.gif" width="1" height="1" /></td>
		</tr>
		...
		<!-- Table body -->
		<logic:iterate id="item" name="<%=BeanKeys.PUNITIVE_MEASURE_LIST%>"
			type="com.vtb.domain.PunitiveMeasure">
			<logic:present name="item">
				<tr id='<bean:write name="item" property="id" filter="true"/>'
					...
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/punitiveMeasureDialog.do?action=Edit" paramId="id" paramName="item" paramProperty="id" paramScope="page">
								<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
						<% } else { %>
							<html:link page="/punitiveMeasureDialog.do?action=View" paramId="id" paramName="item" paramProperty="id" paramScope="page">
					...
					<td class="text" style="text-align: center; vertical-align: middle">
					<% if(canEdit) { %>
						<html:link page="/punitiveMeasureDialog.do?action=Delete" paramId="id"	paramName="item" paramProperty="id" paramScope="page">
