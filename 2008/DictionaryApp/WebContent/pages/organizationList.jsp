...
<%
 	...
 	OrganizationListForm listForm = (OrganizationListForm)session.getAttribute("OrganizationListForm");
 	String sortBy = listForm.getOrderBy();
 	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.ORGANIZATION_LIST);
	...
	int pages = (posListSize%pageSize == 0) ? (posListSize/pageSize) : (posListSize/pageSize + 1);//кол-во страниц. 	
 	//диалог "информация о клииенте":
	//String dialogProperty = "top=100, left=450, width=800, height=400, scrollbars=yes, resizable=yes";
...
<html:form action="/organizationList.do" styleId="listForm">
<html:errors/>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
	...
	<tr class="layout-manager">
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<html:link page="/organizationDialog.do?action=Add">
    ...
    <tr class="title">
		...
		<TD class="column-head-name">Группы</TD>
		<TD class="column-head-name">Адреса</TD>
		<TD class="column-head-name">Управление</TD>
		<TD class="column-head-name">Акционеры</TD>
		<TD class="column-head-name">Контакты</TD>
		<TD class="column-head-name">ОКВЕД</TD>
        ...
        <!-- Table body -->
    <logic:iterate id="item" name="<%=BeanKeys.ORGANIZATION_LIST%>" type="com.vtb.custom.OrganizationTO">
        <logic:present name="item">
        	<logic:present name="item" property="spoID">
	            <tr id='<bean:write name="item" property="spoID" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
	            	<td class="text">
		            	<% if(canEdit) { %>
		                    <html:link page="/organizationDialog.do?action=Edit" paramId="id" paramName="item" paramProperty="spoID" paramScope="page">            	
		                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
		                <% } else { %>
		                    <html:link page="/organizationDialog.do?action=View" paramId="id" paramName="item" paramProperty="spoID" paramScope="page">            	
	                ...
	                <td class="text">
	                    <html:link page="/organizationInfo.do?action=View" paramId="id" paramName="item" paramProperty="spoID" paramScope="page" onclick="return openDialog(this.href, 'organizationInfo', 'top=50, left=350, width=350, height=600, scrollbars=yes, resizable=yes');">            	
	                ...
	                <td class="text">
	                    <html:link page="/organizationToGroupList.do" paramId="organizationID" paramName="item" paramProperty="spoID" paramScope="page">
	                        Группы</html:link>
	                </td>                
	                <td class="text">
	                    <html:link page="/organizationToAddressList.do" paramId="organizationID" paramName="item" paramProperty="spoID" paramScope="page">
	                        Адреса</html:link>
	                </td>
	                <td class="text">
	                    <html:link page="/organizationToGovernList.do" paramId="organizationID" paramName="item" paramProperty="spoID" paramScope="page">
	                        Управление</html:link>
	                </td>
	                <td class="text">
	                    <html:link page="/organizationToShareholderList.do" paramId="organizationID" paramName="item" paramProperty="spoID" paramScope="page">
	                        Акционеры</html:link>
	                </td>
	                <td class="text">
	                    <html:link page="/organizationToManagerList.do" paramId="organizationID" paramName="item" paramProperty="spoID" paramScope="page">
	                        Контакты</html:link>
	                </td>
	                <td class="text">
	                    <html:link page="/organizationToOkvedList.do" paramId="organizationID" paramName="item" paramProperty="spoID" paramScope="page">
	                        ОКВЭД</html:link>
	                </td>	                	                                                                
	                <td class="text">
		                <% if(canEdit) { %>
							<html:link page="/organizationDialog.do?action=Delete" paramId="id" paramName="item" paramProperty="spoID" paramScope="page">
            ...
            <logic:notPresent name="item" property="spoID">
            	<tr id='crm<bean:write name="item" property="crmID" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
					<td class="text">
		            	<% if(canEdit) { %>
		                    <html:link page="/organizationCrmDialog.do?action=Edit" paramId="idCRM" paramName="item" paramProperty="crmID" paramScope="page">            	
		                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
		                <% } else { %>
		                    <html:link page="/organizationCrmDialog.do?action=View" paramId="idCRM" paramName="item" paramProperty="crmID" paramScope="page">            	
		                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Просмотр записи'></html:link>
		                <% } %>					
					</td>
		            ...
		            <td class="text"><bean:write name="item" property="fullName" filter="true"/></td>
		            <td class="text">
	                    <html:link page="/organizationCrmToGroupList.do" paramId="orgCrmID" paramName="item" paramProperty="crmID" paramScope="page">
	                        Группы</html:link>		            
					</td>
		            <td class="text">&nbsp;</td>
		            <td class="text">&nbsp;</td>
