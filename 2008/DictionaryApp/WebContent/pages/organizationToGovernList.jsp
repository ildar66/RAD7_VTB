...
<H3 id='bread-crumb'><html:link page="/organizationList.do" styleClass="navigator">Справочник организаций</html:link> &gt;
	Управление для организации: <I><bean:write name="OrganizationToGovernListForm" property="organizationVO.account_name"/></I> &gt;
</H3><br>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
    ...
    <tr class="title">
		...
		<TD class="column-head-name">
			<A title="Сортировать по полю 'name'" href="javascript:SortBy('name')">название органа</A>
			<%=isUp(sortBy, "name")%>
		</TD>
		<TD class="column-head-name">
			<A title="Сортировать по полю 'docType'" href="javascript:SortBy('docType')">тип документа</A>
			<%=isUp(sortBy, "docType")%>
		</TD>
		<TD class="column-head-name">
			<A title="Сортировать по полю 'activityPeriod'" href="javascript:SortBy('activityPeriod')">Периодичность действия</A>
			<%=isUp(sortBy, "activityPeriod")%>
		</TD>
		<TD class="column-head-name">
			<A title="Сортировать по полю 'isWithInAccount'" href="javascript:SortBy('isWithInAccount')">Присутствие</A>
