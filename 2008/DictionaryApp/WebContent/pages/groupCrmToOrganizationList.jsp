...
<script language="javascript">
	function DeleteLink(id) {
		if(confirm('Исключить органицию SPO №: '+ id +'?')) {
			...
			listForm.operation.value = "<%= GroupCrmToOrganizationListAction.DELETE_LINK%>";
			listForm.submit();
			return false;
		}
	}
	function DeleteLinkCRM(idCRM) {
		if(confirm('Исключить органицию CRM №: '+ idCRM +'?')) {
			...
			listForm.operation.value = "<%= GroupCrmToOrganizationListAction.DELETE_LINK_CRM%>";
			listForm.submit();
			return false;
		}		
	}
	function AddLink() {
		if(listForm.selectedID.value != ''){
			listForm.operation.value = "<%= GroupCrmToOrganizationListAction.ADD_LINK%>";
			document.body.style.cursor="wait";
			storeSelectedRow(listForm.selectedID.value);
			listForm.submit();
		}else{
			listForm.operation.value = "<%= GroupCrmToOrganizationListAction.ADD_LINK_CRM%>";
			document.body.style.cursor="wait";
...
<%
 	...
 	GroupCrmToOrganizationListForm listForm = (GroupCrmToOrganizationListForm)request.getAttribute("GroupCrmToOrganizationListForm");
...
<H3 id='bread-crumb'><html:link page="/organizationGroupList.do" styleClass="navigator">Группы клиентов</html:link> &gt;
	Организации для группы: <I><bean:write name="GroupCrmToOrganizationListForm" property="groupCrmVO.name"/></I> &gt;
</H3><br>
<html:form action="/groupCrmToOrganizationList.do" styleId="listForm">
<html:errors/>
<html:hidden property="groupCrmID"/>
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<A	href='<%=request.getContextPath()%>/organizationLookupList.do?formName=GroupCrmToOrganizationListForm&fieldNames=selectedID|name|selectedCrmID&onMySelect=AddLink()' 
