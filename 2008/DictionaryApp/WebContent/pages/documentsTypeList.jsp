...
<%
	...
	DocumentsTypeListForm listForm = (DocumentsTypeListForm)session.getAttribute("DocumentsTypeListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.DOCUMENT_TYPE_LIST);
...
<html:form action="/documentsTypeList.do" styleId="listForm">
<html:errors/>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
	...
	<TR class="layout-manager">
		<TD colspan="<%= gColSpan %>">
			<table class="framing-table">
				...
				<tr>
					<th class="find-filter">типов документов:</th>
	...
	<tr class="layout-manager">
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<html:link page="/documentsTypeDialog.do?action=Add">
    ...
    <tr class="title">
		...
		<TD class="column-head-name" nowrap>
			<A title="Сортировать по полю 'name'" href="javascript:SortBy('name')">Наименование типов документов</A>
			<%=isUp(sortBy, "name")%>
		</TD>
		<TD class="column-head-name" nowrap>
			<A title="Сортировать по полю 'groupName'" href="javascript:SortBy('groupName')">Группа</A>
			<%=isUp(sortBy, "groupName")%>
		</TD>
        <TD class="column-head-name">&nbsp;</TD>
    </tr>

    <!-- Table body -->
    <logic:iterate id="item" name="<%=BeanKeys.DOCUMENT_TYPE_LIST%>" type="com.vtb.custom.DocumentTypeTO">
        <logic:present name="item">
            <tr id='<bean:write name="item" property="id" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
            	<td class="text">
            	<% if(canEdit) { %>
                    <html:link page="/documentsTypeDialog.do?action=Edit" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
                <% } else { %>
                    <html:link page="/documentsTypeDialog.do?action=View" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
                ...
                <td class="text">
					<bean:write name="item" property="groupName" filter="true"/>
                </td>
                <td class="text">
					<% if(canEdit) { %>
						<html:link	page="/documentsTypeDialog.do?action=Delete" paramId="id" paramName="item" paramProperty="id" paramScope="page">
