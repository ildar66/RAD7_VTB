...
<%
	...
	DocumentGroupListForm listForm = (DocumentGroupListForm)session.getAttribute("DocumentGroupListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.DOCUMENT_GROUP_LIST);
...
<html:form action="/documentGroupList.do" styleId="listForm" focus="searchStr">
	<html:errors />
	...
	<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1">
		...
		<tr CLASS="layout-manager">
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
				<TR>
					<TD class="find-filter">
						<% if(canEdit) { %>
							<html:link page="/documentGroupDialog.do?action=Add">
		...
		<tr class="title">
			<td class="column-head-name" align="center" valign="middle">
				<img src="<%=request.getContextPath()%>/images/empty.gif" width="1" height="1" /></td>
			<td class="column-head-name" nowrap>
				<A title="Сортировать по полю 'id'" href="javascript:SortBy('id')">&nbsp;№&nbsp;</A>
				<%=isUp(sortBy, "id")%></td>				
			<td class="column-head-name" align="center" valign="middle">
				<A title="Сортировать по полю 'name'" href="javascript:SortBy('name')">Группа документа</A>
				<%=isUp(sortBy, "name")%></td>
			<td class="column-head-name" align="center" valign="middle">
				<img src="<%=request.getContextPath()%>/images/empty.gif" width="1" height="1" /></td>
		</tr>
		...
		<!-- Table body -->
		<logic:iterate id="item" name="<%=BeanKeys.DOCUMENT_GROUP_LIST%>"
			type="com.vtb.domain.DocumentGroup">
			<logic:present name="item">
				<tr id='<bean:write name="item" property="id_document_group" filter="true"/>'
					...
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/documentGroupDialog.do?action=Edit" paramId="id"	paramName="item" paramProperty="id_document_group" paramScope="page">
								<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
						 <% } else { %>
							<html:link page="/documentGroupDialog.do?action=View" paramId="id"	paramName="item" paramProperty="id_document_group" paramScope="page">
								<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Просмотр записи'></html:link>
						<% } %>
					</td>
	                <td class="text">
	                    <bean:write name="item" property="id_document_group" filter="true"/></td>
					<td class="text" style="text-align: left; vertical-align: middle">
						<bean:write name="item" property="name_document_group" /></td>
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/documentGroupDialog.do?action=Delete" paramId="id" paramName="item" paramProperty="id_document_group" paramScope="page">
