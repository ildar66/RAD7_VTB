...
<%
	...
	StopFactorListForm listForm = (StopFactorListForm)session.getAttribute("StopFactorListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.STOPFACTOR_LIST);
...
%>
<H3 id='bread-crumb'>
	Справочник стоп-факторов &gt;
</H3><br>
<html:form action="/stopFactorList.do" styleId="listForm" focus="searchStr">
	<html:errors />
	...
	<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1">
		...
		<TR CLASS="layout-manager">
			...
			<table class="framing-table">
				<tr>
					<th class="find-filter">Поиск по коду:</th>
		...
		<tr CLASS="layout-manager">
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
				<TR>
					<TD class="find-filter">
						<% if(canEdit) { %>
							<html:link page="/stopFactorDialog.do?action=Add">
		...
		<!-- Table body -->
		<logic:iterate id="item" name="<%=BeanKeys.STOPFACTOR_LIST%>"
			type="com.vtb.domain.StopFactor">
			<logic:present name="item">
				<tr id='<bean:write name="item" property="code" filter="true"/>'
					...
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/stopFactorDialog.do?action=Edit" paramId="code"	paramName="item" paramProperty="code" paramScope="page">
								<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
						<% } else { %>					
							<html:link page="/stopFactorDialog.do?action=View" paramId="code"	paramName="item" paramProperty="code" paramScope="page">
					...
					<td class="text" style="text-align: left; vertical-align: middle">
						<bean:write name="item" property="code" /></td>
					<td class="text" style="text-align: left; vertical-align: middle">
						<bean:write name="item" property="description" /></td>
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/stopFactorDialog.do?action=Delete" paramId="code" paramName="item" paramProperty="code" paramScope="page">
