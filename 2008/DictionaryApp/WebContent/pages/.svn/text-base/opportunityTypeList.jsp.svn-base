...
<HEAD>
	...
	<TITLE>Справочник вида сделок</TITLE>
</HEAD>
<BODY class="content">
	<SCRIPT language=javascript src="../common/dialog.js" type="text/javascript"></SCRIPT> 
<%
	java.lang.String gColSpan = "4";
	OpportunityTypeListForm listForm = (OpportunityTypeListForm)session.getAttribute("OpportunityTypeListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.OPPORTUNITY_TYPE_LIST);
...
%>
<H3 id='bread-crumb'>
	Справочник вида сделок &gt;
</H3><br>
<html:form action="/opportunityTypeList.do" styleId="listForm">
<html:errors/>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1">
	...
	<TR class="layout-manager">
		<TD colspan="<%= gColSpan %>">
			<table class="framing-table">
				...
				<tr>
					<th class="find-filter">вида сделки:</th>
	...
	<tr class="layout-manager">
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<html:link page="/opportunityTypeDialog.do?action=Add">
    ...
    <tr class="title">
		...
		<TD class="column-head-name">
			<A title="Сортировать по полю 'name'" href="javascript:SortBy('name')">Наименование вида сделки</A>
			<%=isUp(sortBy, "name")%>
		</TD>
        ...
        <!-- Table body -->
    <logic:iterate id="item" name="<%=BeanKeys.OPPORTUNITY_TYPE_LIST%>" type="com.vtb.domain.OpportunityType">
        <logic:present name="item">
            <tr id='<bean:write name="item" property="id" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
            	<td class="text">
            	<% if(canEdit) { %>
                    <html:link page="/opportunityTypeDialog.do?action=Edit" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
                <% } else { %>
                    <html:link page="/opportunityTypeDialog.do?action=View" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
                ...
                <td class="text">
                    <% if(canEdit) { %>
						<html:link 
						page="/opportunityTypeDialog.do?action=Delete" paramId="id" paramName="item"
