...
<HEAD>
	...
	<TITLE>Адреса для организации</TITLE>
</HEAD>
<BODY class="content">

<H3 id='bread-crumb'><html:link page="/organizationList.do" styleClass="navigator">Справочник организаций</html:link> &gt;
	Адреса для организации: <I><bean:write name="OrganizationToAddressListForm" property="organizationVO.account_name"/></I> &gt;
</H3><br>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
    ...
    <tr class="title">
		...
		<TD class="column-head-name">
			<A title="Сортировать по полю 'city'" href="javascript:SortBy('city')">город</A>
			<%=isUp(sortBy, "city")%>
		</TD>
		<TD class="column-head-name">
			<A title="Сортировать по полю 'postalCode'" href="javascript:SortBy('postalCode')">индекс</A>
			<%=isUp(sortBy, "postalCode")%>
		</TD>
		<TD class="column-head-name">
			<A title="Сортировать по полю 'country'" href="javascript:SortBy('country')">страна</A>
			<%=isUp(sortBy, "country")%>
		</TD>
		<TD class="column-head-name">
			<A title="Сортировать по полю 'county'" href="javascript:SortBy('county')">округ</A>
			<%=isUp(sortBy, "county")%>
		</TD>
		<TD class="column-head-name">
