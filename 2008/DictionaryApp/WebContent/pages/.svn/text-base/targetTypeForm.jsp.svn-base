...
<BODY class="content">
<%
	String form = "TargetTypeDialogForm";
	String src = request.getContextPath() + "/images/save.gif";
	String alt = "Сохранить запись";
%>

<H3 id='bread-crumb'><html:link page="/targetTypeList.do" styleClass="navigator">Справочник целей кредитования</html:link> &gt;
	<bean:write name="<%= form%>"/> :
</H3><br>

<html:form action="/targetTypeProcess.do" focus="vo.name">
<html:errors/>
...
<script language="javascript">
	...
	function storeID() {
		document.cookie = "TargetTypeListForm=" + document.<%= form %>['vo.id'].value;
	}	
</script>
...
<table class="wizard-table" border="0" cellpadding="3" cellspacing="0">
    ...
    <tr>
    	<td class="column-head-name" colspan="2">
			...
			<html:link page="/targetTypeList.do" styleClass="navigator">
	...
	<tr>
		<td colspan="2">
			<table>
			    <tr>
			        <td class="text" colspan="2">№ целей кредитования:</td>
			        <td class="text">
			    ...
			    <tr>
			        <td class="text" colspan="2">Имя целей кредитования:</td>
			        <td class="text">
