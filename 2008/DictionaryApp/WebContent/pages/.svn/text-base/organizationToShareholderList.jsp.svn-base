...
<H3 id='bread-crumb'><html:link page="/organizationList.do" styleClass="navigator">Справочник организаций</html:link> &gt;
	Акционеры для организации: <I><bean:write name="OrganizationToShareholderListForm" property="organizationVO.account_name"/></I> &gt;
</H3><br>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
    ...
    <tr class="title">
		...
		<TD class="column-head-name">
			<A title="Сортировать по полю 'fio'" href="javascript:SortBy('fio')">Название владельца/акционера</A>
			<%=isUp(sortBy, "fio")%>
		</TD>
		<TD class="column-head-name">
			<A title="Сортировать по полю 'category'" href="javascript:SortBy('category')">Категория</A>
			<%=isUp(sortBy, "category")%>
		</TD>
		<TD class="column-head-name">
			<A title="Сортировать по полю 'part'" href="javascript:SortBy('part')">Процент участия</A>
			<%=isUp(sortBy, "part")%>
		</TD>
		<TD class="column-head-name" align="center" valign="middle">
			<img src="<%=request.getContextPath()%>/images/empty.gif" width="1" height="1" />
