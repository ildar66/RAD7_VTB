...
<HEAD>
	...
	<TITLE>Организационно-правовые формы</TITLE>
</HEAD>
<BODY class="content">
...
<%
	...
	OwnershipFormTypeListForm listForm = (OwnershipFormTypeListForm)session.getAttribute("OwnershipFormTypeListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.OWNERSHIP_FORM_TYPE_LIST);
	int posListSize = posList.size();//кол-во позиций.
	//paging:
	int pageSize = (posListSize > 300)? 25 : 21 ;//кол-во позиций на странице.
...
<html:form action="/ownershipFormTypeList.do" styleId="listForm">
<html:errors/>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
	...
	<TR class="layout-manager">
		<TD colspan="<%= gColSpan %>">
			<table class="framing-table" cellpadding="2">
				<tr>
					<th class="find-filter">Наименование организационно-<br>правовой формы:</th>
	...
	<tr class="layout-manager">
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<html:link page="/ownershipFormTypeDialog.do?action=Add">
    ...
    <tr class="title">
		...
		<TD class="column-head-name" nowrap>
			<A title="Сортировать по полю 'code'" href="javascript:SortBy('code')">Код OPF</A>
			<%=isUp(sortBy, "code")%>
		</TD>		
		<TD class="column-head-name" nowrap>
		...
		<TD class="column-head-name" nowrap>
			<A title="Сортировать по полю 'association'" href="javascript:SortBy('association')">Принадлежность</A>
			<%=isUp(sortBy, "association")%>
		</TD>
        <TD class="column-head-name">&nbsp;</TD>
    </tr>

    <!-- Table body -->
    <logic:iterate id="item" name="<%=BeanKeys.OWNERSHIP_FORM_TYPE_LIST%>" type="com.vtb.domain.OwnershipFormType">
        <logic:present name="item">
            <tr id='<bean:write name="item" property="id" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
            	<td class="text">
	            	<% if(canEdit) { %>
	                    <html:link page="/ownershipFormTypeDialog.do?action=Edit" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
	                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
	                <% } else { %>
	                    <html:link page="/ownershipFormTypeDialog.do?action=View" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
                ...
                <td class="text">
					<bean:write name="item" property="associationStr" filter="true"/>
                </td>
                <td class="text">
	                <% if(canEdit) { %>
						<html:link page="/ownershipFormTypeDialog.do?action=Delete" paramId="id" paramName="item" paramProperty="id" paramScope="page">
