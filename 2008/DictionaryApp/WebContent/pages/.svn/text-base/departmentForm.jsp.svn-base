...
<BODY class="content">
<%
	String form = "DepartmentDialogForm";
...
<html:form action="/departmentProcess.do" focus="vo.fullName">
<html:errors/>
...
<script language="javascript">
	function Check(){
		...
		with(theForm){
			...
			if(!field_check(theForm['vo.fullName'], 'Полное имя',0,true)) return false;
			if(!field_check(theForm['vo.shortName'], 'Короткое имя',0,true)) return false;
			if(!field_check(theForm['vo.code'], 'Код', 3, false)) return false;
		}
		storeID();
		return true;
	}
	function storeID() {
		document.cookie = "DepartmentListForm=" + document.<%= form %>['vo.id'].value;
	}	
</script>
		...
		<td colspan="2">
			<table>
			    <tr>
			        ...
			        <td class="text">
			          <html:text property="vo.code" size="20" readonly="false"/>
			        </td>
			    </tr>

			    <tr>
			        <td class="text" colspan="2">Полное имя филиала:</td>
			        <td class="text">
			          <html:text property="vo.fullName" size="50" />
			        </td>
			    </tr>
			    
			    <tr>
			        <td class="text" colspan="2">Короткое имя филиала:</td>
			        <td class="text">
			          <html:text property="vo.shortName" size="50" />
			        </td>
			    </tr>
			</table>
		</td>
