...
<BODY class="content">
<%
	String form = "ContractorTypeDialogForm";
	String src = request.getContextPath() + "/images/save.gif";
	String alt = "Сохранить запись";
%>

<H3 id='bread-crumb'><html:link page="/contractorTypeList.do" styleClass="navigator">Справочник типов контрагентов</html:link> &gt;
	<bean:write name="<%= form%>"/> :
</H3><br>

<html:form action="/contractorTypeProcess.do" focus="vo.name">
<html:errors/>
...
<script language="javascript">
	...
	function storeID() {
		document.cookie = "ContractorTypeListForm=" + document.<%= form %>['vo.id'].value;
	}	
</script>
...
<table class="wizard-table" border="0" cellpadding="3" cellspacing="0">
    ...
    <tr>
    	<td class="column-head-name" colspan="2">
			...
			<html:link page="/contractorTypeList.do" styleClass="navigator">
	...
	<tr>
		<td colspan="2">
			<table>
			    <tr>
			        <td class="text" colspan="2">№ типов контрагентов:</td>
			        <td class="text">
			    ...
			    <tr>
			        <td class="text" colspan="2">Имя типов контрагентов:</td>
			        <td class="text">
