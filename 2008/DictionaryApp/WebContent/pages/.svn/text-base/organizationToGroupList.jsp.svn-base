...
<HEAD>
	...
	<TITLE>Группы, в состав которых входит организация</TITLE>
</HEAD>
<BODY class="content">
...
<script language="javascript">
	function DeleteLink(id) {
		if(confirm('Исключить организацию № '+ id +'?')) {
			...
			listForm.operation.value = "<%= OrganizationToGroupListAction.DELETE_LINK%>";
			listForm.submit();
	...
	function DeleteLinkCRM(idCRM) {
		if(confirm('Исключить организацию CRM № '+ idCRM +'?')) {
			...
			listForm.operation.value = "<%= OrganizationToGroupListAction.DELETE_LINK_CRM%>";
			listForm.submit();
	...
	function AddLink() {
		if(listForm.selectedID.value != ''){
			listForm.operation.value = "<%= OrganizationToGroupListAction.ADD_LINK%>";
			document.body.style.cursor="wait";
			storeSelectedRow(listForm.selectedID.value);
			listForm.submit();
		}else{
			listForm.operation.value = "<%= OrganizationToGroupListAction.ADD_LINK_CRM%>";
			document.body.style.cursor="wait";
...
<%
 	...
 	OrganizationToGroupListForm listForm = (OrganizationToGroupListForm)request.getAttribute("OrganizationToGroupListForm");
...
<H3 id='bread-crumb'><html:link page="/organizationList.do" styleClass="navigator">Справочник организаций</html:link> &gt;
	Группы, в состав которых входит организация: <I><bean:write name="OrganizationToGroupListForm" property="organizationVO.account_name"/></I> &gt;
</H3><br>
<html:form action="/organizationToGroupList.do" styleId="listForm">
<html:errors/>
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<A	href='<%=request.getContextPath()%>/organizationGroupLookupList.do?formName=OrganizationToGroupListForm&fieldNames=selectedID|name|selectedCrmID&onMySelect=AddLink()'
