...
<HEAD>
	...
	<TITLE>Порядок уплаты процентов</TITLE>
</HEAD>
<BODY class="content">
<%
	java.lang.String gColSpan = "4";
	PatternPaidPercentTypeListForm listForm = (PatternPaidPercentTypeListForm)session.getAttribute("PatternPaidPercentTypeListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.PATTERN_PAID_PERCENT_TYPE_LIST);
...
<html:form action="/patternPaidPercentTypeList.do" styleId="listForm" focus="searchStr">
	<html:errors />
	...
	<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1">
		...
		<TR CLASS="layout-manager">
			...
			<table class="framing-table">
				<tr>
					<th class="find-filter">Поиск по наименованию:</th>
		...
		<tr CLASS="layout-manager">
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
				<TR>
					<TD class="find-filter">
						<% if(canEdit) { %>
							<html:link page="/patternPaidPercentTypeDialog.do?action=Add">
		...
		<tr class="title">
			...
			<td class="column-head-name" align="center" valign="middle"><A
				...
				href="javascript:SortBy('name')">Наименование способа(порядка) уплаты процентов</A>
				<%=isUp(sortBy, "name")%></td>
			<td class="column-head-name" align="center" valign="middle"><img
				src="<%=request.getContextPath()%>/images/empty.gif" width="1" height="1" /></td>
		</tr>
		...
		<!-- Table body -->
		<logic:iterate id="item" name="<%=BeanKeys.PATTERN_PAID_PERCENT_TYPE_LIST%>"
			type="com.vtb.domain.PatternPaidPercentType">
			<logic:present name="item">
				<tr id='<bean:write name="item" property="id" filter="true"/>'
					...
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/patternPaidPercentTypeDialog.do?action=Edit" paramId="id"	paramName="item" paramProperty="id" paramScope="page">
								<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
						<% } else { %>
							<html:link page="/patternPaidPercentTypeDialog.do?action=View" paramId="id"	paramName="item" paramProperty="id" paramScope="page">
								<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Просмотр записи'></html:link>
						<% } %>
						
					</td>
	                <td class="text">
	                    <bean:write name="item" property="id" filter="true"/></td>
					<td class="text" style="text-align: left; vertical-align: middle">
						<bean:write name="item" property="name" /></td>
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/patternPaidPercentTypeDialog.do?action=Delete" paramId="id"
								paramName="item" paramProperty="id" paramScope="page">
