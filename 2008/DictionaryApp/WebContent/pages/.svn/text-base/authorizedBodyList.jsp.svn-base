...
<%
	...
	AuthorizedBodyListForm listForm = (AuthorizedBodyListForm)session.getAttribute("AuthorizedBodyListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.AUTHORIZED_BODY_LIST);
...
<html:form action="/authorizedBodyList.do" styleId="listForm" focus="searchStr">
	<html:errors />
	...
	<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1">
		...
		<tr CLASS="layout-manager">
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
				<TR>
					<TD class="find-filter">
						<% if(canEdit) { %>
							<html:link page="/authorizedBodyDialog.do?action=Add">
		...
		<tr class="title">
			...
			<td class="column-head-name" align="center" valign="middle"><A
				...
				href="javascript:SortBy('name')">Наименование Уполномоченного органа</A>
				<%=isUp(sortBy, "name")%></td>
			<td class="column-head-name" align="center" valign="middle"><A
				title="Сортировать по полю 'isColegial'"
				href="javascript:SortBy('isColegial')">Является коллегиальным?</A>
				<%=isUp(sortBy, "isColegial")%></td>
			<td class="column-head-name" align="center" valign="middle"><A
				title="Сортировать по полю 'isGeneral'"
				href="javascript:SortBy('isGeneral')">Относится к ГО?</A>
				<%=isUp(sortBy, "isGeneral")%></td>
			<td class="column-head-name" align="center" valign="middle"><img
				src="<%=request.getContextPath()%>/images/empty.gif" width="1" height="1" /></td>
		</tr>
		...
		<!-- Table body -->
		<logic:iterate id="item" name="<%=BeanKeys.AUTHORIZED_BODY_LIST%>"
			type="com.vtb.domain.AuthorizedBody">
			<logic:present name="item">
				<tr id='<bean:write name="item" property="id" filter="true"/>'
					...
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/authorizedBodyDialog.do?action=Edit" paramId="id"	paramName="item" paramProperty="id" paramScope="page">
								<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
						<% } else { %>
							<html:link page="/authorizedBodyDialog.do?action=View" paramId="id"	paramName="item" paramProperty="id" paramScope="page">
					...
					<td class="text" style="text-align: left; vertical-align: middle">
                        <logic:equal name="item" property="isColegial" value="true">ДА</logic:equal>
						<logic:equal name="item" property="isColegial" value="false">НЕТ</logic:equal>
					</td>
					<td class="text" style="text-align: left; vertical-align: middle">
                        <logic:equal name="item" property="isGeneral" value="true">ДА</logic:equal>
						<logic:equal name="item" property="isGeneral" value="false">НЕТ</logic:equal>
					</td>
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/authorizedBodyDialog.do?action=Delete" paramId="id" paramName="item" paramProperty="id" paramScope="page">
