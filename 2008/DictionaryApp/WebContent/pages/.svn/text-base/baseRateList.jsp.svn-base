...
<HEAD>
	...
	<TITLE>Справочник базовых ставок</TITLE>
</HEAD>
<BODY class="content">
...
<%
	...
	BaseRateListForm listForm = (BaseRateListForm)session.getAttribute("BaseRateListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.BASE_RATE_LIST);
...
%>
<H3 id='bread-crumb'>
	Справочник базовых ставок &gt;
</H3><br>
<html:form action="/baseRateList.do" styleId="listForm">
<html:errors/>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
	...
	<TR class="layout-manager">
		<TD colspan="<%= gColSpan %>">
			<table class="framing-table" cellpadding="2">
				...
				<tr>
					<th class="find-filter">базовой ставки:</th>
	...
	<tr class="layout-manager">
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<html:link page="/baseRateDialog.do?action=Add">
    ...
    <tr class="title">
		...
		<TD class="column-head-name" nowrap>
			<A title="Сортировать по полю 'name'" href="javascript:SortBy('name')">Наименование БС</A>
			<%=isUp(sortBy, "name")%>
		</TD>
		<TD class="column-head-name" nowrap>
    ...
    <!-- Table body -->
    <logic:iterate id="item" name="<%=BeanKeys.BASE_RATE_LIST%>" type="com.vtb.domain.BaseRate">
        <logic:present name="item">
            <tr id='<bean:write name="item" property="id" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
            	<td class="text">
	            	<% if(canEdit) { %>
	                    <html:link page="/baseRateDialog.do?action=Edit" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
	                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
	                <% } else { %>
	                    <html:link page="/baseRateDialog.do?action=View" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
                ...
                <td class="text">
                    <% if(canEdit) { %>
						<html:link	page="/baseRateDialog.do?action=Delete" paramId="id" paramName="item" paramProperty="id" paramScope="page">
