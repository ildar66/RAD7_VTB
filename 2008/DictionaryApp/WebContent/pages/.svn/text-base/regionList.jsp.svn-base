...
<%
	...
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.REGION_LIST);
	int posListSize = posList.size();//кол-во позиций.
	//paging:
	int pageSize = (posListSize > 300)? 30 : 23 ;//кол-во позиций на странице.
...
<html:form action="/regionList.do" styleId="listForm">
<html:errors/>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
	...
	<TR class="layout-manager">
		<TD colspan="<%= gColSpan %>">
			<table class="framing-table">
				...
				<tr>
					<th class="find-filter">региона:</th>
	...
	<tr class="layout-manager">
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<html:link page="/regionDialog.do?action=Add">
    ...
    <tr class="title">
		...
		<TD class="column-head-name" nowrap>
			<A title="Сортировать по полю 'name'" href="javascript:SortBy('name')">Наименование региона</A>
			<%=isUp(sortBy, "name")%>
		</TD>
		<TD class="column-head-name" nowrap>
			<A title="Сортировать по полю 'rating'" href="javascript:SortBy('rating')">Рейтинг</A>
			<%=isUp(sortBy, "rating")%>
		</TD>
    ...
    <!-- Table body -->
    <logic:iterate id="item" name="<%=BeanKeys.REGION_LIST%>" type="com.vtb.domain.Region">
        <logic:present name="item">
            <tr id='<bean:write name="item" property="id" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
            	<td class="text">
	            	<% if(canEdit) { %>
	                    <html:link page="/regionDialog.do?action=Edit" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
	                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
	                <% } else { %>
	                    <html:link page="/regionDialog.do?action=View" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
                ...
                <td class="text">
                    <% if(canEdit) { %>
						<html:link 
						page="/regionDialog.do?action=Delete" paramId="id" paramName="item"
