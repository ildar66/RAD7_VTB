...
<HEAD>
	...
	<TITLE>Справочник целей кредитования</TITLE>
</HEAD>
<BODY class="content">
...
<%
	...
	TargetTypeListForm listForm = (TargetTypeListForm)session.getAttribute("TargetTypeListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.TARGET_TYPE_LIST);
...
%>
<H3 id='bread-crumb'>
	Справочник целей кредитования &gt;
</H3><br>
<html:form action="/targetTypeList.do" styleId="listForm">
<html:errors/>
...
<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1" width="100%">
	...
	<TR class="layout-manager">
		<TD colspan="<%= gColSpan %>">
			<table class="framing-table">
				...
				<tr>
					<th class="find-filter">цели кредитования:</th>
	...
	<tr class="layout-manager">
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
			<TR>
				<TD class="find-filter">
					<% if(canEdit) { %>
						<html:link page="/targetTypeDialog.do?action=Add">
    ...
    <tr class="title">
		...
		<TD class="column-head-name">
			<A title="Сортировать по полю 'name'" href="javascript:SortBy('name')">Наименование цели кредитования</A>
			<%=isUp(sortBy, "name")%>
		</TD>
        ...
        <!-- Table body -->
    <logic:iterate id="item" name="<%=BeanKeys.TARGET_TYPE_LIST%>" type="com.vtb.domain.TargetType">
        <logic:present name="item">
            <tr id='<bean:write name="item" property="id" filter="true"/>' class="normal" onMouseOver="className='select'" onMouseOut="onMouseOutRow(this.id);" onclick="onClickRow(this.id);">
            	<td class="text">
	            	<% if(canEdit) { %>
	                    <html:link page="/targetTypeDialog.do?action=Edit" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
	                    	<IMG SRC="<%=request.getContextPath()%>/images/izmena.gif" BORDER='0' ALT='Изменить запись'></html:link>
	                <% } else { %>
	                    <html:link page="/targetTypeDialog.do?action=View" paramId="id" paramName="item" paramProperty="id" paramScope="page">            	
                ...
                <td class="text">
                    <% if(canEdit) { %>
						<html:link	page="/targetTypeDialog.do?action=Delete" paramId="id" paramName="item"	paramProperty="id" paramScope="page">
