...
<HEAD>
	...
	<TITLE>Справочник рейтингов</TITLE>
</HEAD>
<BODY class="content">
<%
	java.lang.String gColSpan = "4";
	RatingTypeListForm listForm = (RatingTypeListForm)session.getAttribute("RatingTypeListForm");
	String sortBy = listForm.getOrderBy();
	java.util.List posList = (java.util.List)request.getAttribute(BeanKeys.RATING_TYPE_LIST);
...
<html:form action="/ratingTypeList.do" styleId="listForm" focus="searchStr">
	<html:errors />
	...
	<table class="framing-table" id="tableID" BORDER="0" CELLPADDING="3" CELLSPACING="1">
		...
		<tr CLASS="layout-manager">
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1" width="100%">
				<TR>
					<TD class="find-filter">
						<% if(canEdit) { %>
							<html:link page="/ratingTypeDialog.do?action=Add">
		...
		<tr class="title">
			...
			<td class="column-head-name" nowrap>
				...
				<%=isUp(sortBy, "id")%></td>				
			<td class="column-head-name" align="center" valign="middle">
				<A title="Сортировать по полю 'name_rating_type'" href="javascript:SortBy('name_rating_type')">Рейтинг</A>
				<%=isUp(sortBy, "name_rating_type")%></td>
			<td class="column-head-name" align="center" valign="middle">
				<img src="<%=request.getContextPath()%>/images/empty.gif" width="1" height="1" /></td>
		</tr>
		...
		<!-- Table body -->
		<logic:iterate id="item" name="<%=BeanKeys.RATING_TYPE_LIST%>"
			type="com.vtb.domain.RatingType">
			<logic:present name="item">
				<tr id='<bean:write name="item" property="id" filter="true"/>'
					...
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/ratingTypeDialog.do?action=Edit" paramId="id"
						...
						<% } else { %>
							<html:link page="/ratingTypeDialog.do?action=View" paramId="id"
								paramName="item" paramProperty="id" paramScope="page">
					...
					<td class="text" style="text-align: center; vertical-align: middle">
						<% if(canEdit) { %>
							<html:link page="/ratingTypeDialog.do?action=Delete" paramId="id"
								paramName="item" paramProperty="id" paramScope="page">
