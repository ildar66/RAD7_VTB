...
import com.vtb.security.forms.OrganizationLookupListForm;
 ...
 * @author
 */
public class OrganizationLookupListAction extends Action {

	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		ActionForward forward = null;
		try {
			// Look at the ActionForm object
			OrganizationLookupListForm aForm = (OrganizationLookupListForm) form;
			// Get actionprocess (model) and delegate
			SecurityActionProcessor processor = (SecurityActionProcessor) ActionProcessorFactory.getActionProcessor("Security");
			//ArrayList list = processor.findOrganizationList(aForm.getSearchStr(), aForm.getOrderBy());
			ArrayList list = processor.findOrganizationListTO(aForm.getSearchStr(), aForm.getOrderBy());

			// Add ValueBean to request context and forward response
			request.setAttribute(BeanKeys.ORGANIZATION_LOOKUP_LIST, list); 
