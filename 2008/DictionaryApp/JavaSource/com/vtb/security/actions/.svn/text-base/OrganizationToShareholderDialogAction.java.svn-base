 ...
 * @author ildar
 */
public class OrganizationToShareholderDialogAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action) || DialogForm.ACTION_VIEW.equalsIgnoreCase(action) || DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				dialogForm.setVo(processor.findShareholderByKey(dialogForm.getVo().getId().intValue()));
				//находим Организацию:
				Integer organizationKey = dialogForm.getVo().getId_contractor().intValue();
				...
				int id = OIDGeneratorHelper.getNextId("tables.SHAREHOLDER");
				dialogForm.getVo().setId(new Long(id));
				//находим Организацию:
				Integer organizationKey = dialogForm.getVo().getId_contractor().intValue();
				if (organizationKey != null) {
					dialogForm.setOrganizationVO(processor.findOrganizationByKey(organizationKey));
