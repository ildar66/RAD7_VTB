 ...
 * @author
 */
public class IndustryProcessAction extends Action
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				processor.addIndustry(industryDialogForm.getVo());
				industryDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action)) {
				processor.updateIndustry(industryDialogForm.getVo());
				industryDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				processor.removeIndustry(industryDialogForm.getVo().getId());
				industryDialogForm.setFlagOperation(true);
			}
		} catch (Exception e) {
