 ...
 * @author
 */
public class ContractorTypeDialogAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action) || DialogForm.ACTION_VIEW.equalsIgnoreCase(action) || DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				contractorTypeDialogForm.setVo(processor.findContractorTypeByKey(contractorTypeDialogForm.getVo().getId()));
			} else if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				int id = OIDGeneratorHelper.getNextId("tables.CONTRACTOR_TYPE");
				contractorTypeDialogForm.setVo(new ContractorType(new Integer(id), ""));
			}
		} catch (Exception e) {
