 ...
 * @author
 */
public class PatternPaidPercentTypeDialogAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action) || DialogForm.ACTION_VIEW.equalsIgnoreCase(action) || DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				patternPaidPercentTypeDialogForm.setVo(processor.findPatternPaidPercentTypeByKey(patternPaidPercentTypeDialogForm.getVo().getId()));
			} else if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				int id = OIDGeneratorHelper.getNextId("tables.PATTERN_PAID_PERCENT_TYPE");
				patternPaidPercentTypeDialogForm.setVo(new PatternPaidPercentType(new Integer(id), ""));
			}
		} catch (Exception e) {
