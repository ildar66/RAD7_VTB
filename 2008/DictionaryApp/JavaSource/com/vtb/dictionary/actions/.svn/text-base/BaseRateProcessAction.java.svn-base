...
import com.vtb.dictionary.forms.BaseRateDialogForm;
 ...
 * @author
 */
public class BaseRateProcessAction extends Action

{
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		BaseRateDialogForm baseRateDialogForm = (BaseRateDialogForm) form;
		...
		try {
			...
			String action = baseRateDialogForm.getAction();
			if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				processor.addBaseRate(baseRateDialogForm.getVo());
				baseRateDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action)) {
				processor.updateBaseRate(baseRateDialogForm.getVo());
				baseRateDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				processor.removeBaseRate(baseRateDialogForm.getVo().getId());
				baseRateDialogForm.setFlagOperation(true);
			}
		} catch (Exception e) {
