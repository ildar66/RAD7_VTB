...
import com.vtb.dictionary.forms.TargetTypeDialogForm;
 ...
 * @author
 */
public class TargetTypeProcessAction extends Action

{
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		TargetTypeDialogForm targetTypeDialogForm = (TargetTypeDialogForm) form;
		...
		try {
			...
			String action = targetTypeDialogForm.getAction();
			if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				processor.addTargetType(targetTypeDialogForm.getVo());
				targetTypeDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action)) {
				processor.updateTargetType(targetTypeDialogForm.getVo());
				targetTypeDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				processor.removeTargetType(targetTypeDialogForm.getVo().getId());
				targetTypeDialogForm.setFlagOperation(true);
			}
		} catch (Exception e) {
