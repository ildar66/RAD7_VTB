 ...
 * @author
 */
public class RegionProcessAction extends Action
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (DialogForm.ACTION_ADD.equalsIgnoreCase(action)) {
				processor.addRegion(regionDialogForm.getVo());
				regionDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_EDIT.equalsIgnoreCase(action)) {
				processor.updateRegion(regionDialogForm.getVo());
				regionDialogForm.setFlagOperation(true);
			} else if (DialogForm.ACTION_DELETE.equalsIgnoreCase(action)) {
				processor.removeRegion(regionDialogForm.getVo().getId());
				regionDialogForm.setFlagOperation(true);
			}
		} catch (Exception e) {
